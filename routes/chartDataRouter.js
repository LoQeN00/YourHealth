...
router.get('/',async (req,res)=> {
    ...
    const steps24 = await getSteps(30, Date.now() - 86400000 - timeZoneOffset, Date.now(),  token)
    const [steps24_arr, date24_arr] = steps24
    const steps7d = await getSteps(1440, Date.now() - 604800000 - timeZoneOffset, Date.now(), token)
    const [steps7d_arr, date7d_arr] = steps7d
    const steps30d = await getSteps(10080, Date.now() - 2600640000 - timeZoneOffset, Date.now(), token)
